...
export class GrpcClients implements OnModuleInit {
  ...
  onModuleInit() {
    this.restaurantService =
      this.restaurantClient.getService<RestaurantService>('RestaurantService');
    this.groceryService =
      this.groceryClient.getService<GroceryService>('GroceryService');
    this.medicalService =
      this.medicalClient.getService<MedicalService>('MedicalService');
  }

  async getRestaurant(id: string) {
    return await firstValueFrom(
      this.restaurantService.GetRestaurantInfo({ restaurantId: id }),
    );
  }

  async getGrocery(id: string) {
    return await firstValueFrom(
      this.groceryService.GetGroceryInfo({ groceryId: id }),
    );
  }

  async getMedical(id: string) {
    return await firstValueFrom(
      this.medicalService.GetMedicalInfo({ medicalId: id }),
    );
  }
}
